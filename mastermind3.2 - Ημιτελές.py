...
def maingame():
    ...
    gamecanvas = tk.Canvas(root.win, height=452, width=831)
    gamecanvas.place(relx=0.02, rely=0.085)
    canvimg = ImageTk.PhotoImage(Image.open('canvas.png'))
    gamecanvas.create_image(415, 226, image=canvimg)
...
def checkinp():
    ...
    if '' in Li:
        ...
        l=0
        for color in Li:
            ball = gamecanvas.create_oval(l*103, counter*41, l*103+103, counter*41+41, fill=colorhexvals[color])
            l=l+1
        ...
        for output in outputcolors:
            if output == 'RED':
                ball2 = gamecanvas.create_oval(j*103+450, counter*41, j*103+553, counter*41+41, fill='red')
            if output == 'WHITE':
                ball2 = gamecanvas.create_oval(j*103+450, counter*41, j*103+553, counter*41+41, fill='white')
...
colorhexvals = {'Μαύρο':'#000000', 'Άσπρο':'#FFFFFF', 'Μπλε':'#0000FF', 'Κόκκινο':'#FF0000', 'Κίτρινο':'#FFFF00', 'Πράσινο':'#00FF00', 'Πορτοκαλί':'#F92045', 'Μωβ':'#123456'}
file = open('SETTINGS.TXT', 'r', encoding='UTF-8')
