...
def maingame():
    intro.win.destroy()
    global root
    root = Window('Mastermind.py', bgcolor, fgcolor, None, '700x800', 'Παίζει ο παίκτης: {0} :: Ημ/νία: {1}. Επιλέξτε τα χρώματα απο τα combo box'.format(playername[rounds%2], datetime.datetime.now().strftime('%d-%m-%y')))
...
def checkinp():
    ...
    if counter>10:
        root.win.destroy()
        filesc=open('SETTINGS.txt', 'a', encoding='UTF-8')
        filesc.write('{}  {}  - Αποτυχία\n '.format(str(datetime.datetime.now().strftime('%d - %m - %y  --> Παίκτης ')), playername[rounds%2]))
        filesc.close()
        counter=0
        players=1
...
def settings():
    ...
    setdialog = Window('Ρυθμίσεις του περιβάλλοντος', bgcolor, fgcolor, None, '400x300', 'Για να καταχωρηθούν οι αλλαγές, πατήστε Εφαρμογή και επανεκκινήστε το παιχνίδι', ('Εφαρμογή', applysettings))
...
def about():                                                      ### ΕΔΩ ΘΑ ΒΑΛΟΥΜΕ ΤΑ ΟΝΟΜΑΤΑ ΜΑΣ
    aboutdialog = Window('Πανεπιστήμιο Πατρών 2021',bgcolor, fgcolor, None, '400x400', 'Π. Mastermind. \n\nΔυνατότητες: 1. Επιλέγεται ενας συνδυασμος τεσσαρων ΔΙΑΦΟΡΕΤΙΚΩΝ μεταξυ τους χρωμάτων κι ενας παίκτης το μαντευει. 2. Ενας αλλος παίκτης επιλέγει συνδυασμό 3. Δυο παίκτες\
                        ανταγωνίζονται μεταξύ τους και το πρόγραμμα δημιουργεί συνεχώς νεους συνδυασμούς. Το σκορ καταγραφεται σε ΑΡΧΕΙΟ στο οποιο ευκολα αποκτάται πρόσβαση με το τελευταιο κουμπί. \n To παρόν αποτελεί λογισμικό ανοιχτού κώδικα. \nΣυντελεστές: Παναγιώτης Θεοφανίδης,\
                        Κυριάκος Κυριάκου, Κωνσταντίνος Κουσαφιές, Δημήτρης Γαλατσίδας, Σπύρος Αλεξάνδρου \n https://github.com/Theofanidis/Mastermind \n Made with python 3.7.4, NCH Wavepad, Paint for Windows 7')
    
def showscore():
    file = open('SETTINGS.txt', 'r', encoding='UTF-8')
    text = ''.join(file.readlines()[6:]).rstrip()
    scoredialog = Window('Προβολή - Διαγραφή Σκόρ', bgcolor, fgcolor, None, '400x600', text, ('Εκκαθάριση όλων', delscore))
    file.close()
...
def startscreen():
    ...
    lab0 = tk.Label(intro.win, text = "Mastermind", bg='#ffffff', fg='#000000', font=('Times New Roman','23'), padx='20', pady='0')
    ...
    butintro1 = tk.Button(intro.win, text='1 παίκτης - Αυτ/τη επιλογή κωδικού', bg='brown', fg='white', height='1', width='30', command= maingame )
    butintro1.grid(row=2, column=1)
    butintro2 = tk.Button(intro.win, text='2 παίκτες - Χειρ/τη επιλογή κωδικού', bg='brown', fg='white', height='1', width='30', command= twoplayers )
    butintro2.grid(row=3, column=1)
    butintro6 = tk.Button(intro.win, text='2 παίκτες εναλλάξ', bg='brown', fg='white', height='1', width='30', command= alternatingplayers )
    butintro6.grid(row=4, column=1)
    butintro3 = tk.Button(intro.win, text='Ρυθμίσεις', bg='brown', fg='white', height='1', width='20', command= settings )
    butintro3.grid(row=5, column=1)
    butintro4 = tk.Button(intro.win, text='Περί', bg='brown', fg='white', height='1', width='20', command= about)
    butintro4.grid(row=6, column=1)
    butintro5 = tk.Button(intro.win, text='Διαχείριση σκορ', bg='brown', fg='white', height='1', width='20', command= showscore)
